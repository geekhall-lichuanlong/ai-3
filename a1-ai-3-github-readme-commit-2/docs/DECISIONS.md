...
+---------------------------+
|        TenantGuard        |
| validates path tenant_id  |
| allows tenant_a/b/c only  |
+-------------+-------------+
              |
      +-------+-------------------------------+
      |                                       |
      v                                       v
+---------------------------+       +---------------------------+
|        Upload API         |       |        QA SSE API         |
| POST /tenants/{id}/files  |       | POST /tenants/{id}/qa     |
+-------------+-------------+       +-------------+-------------+
              |                                   |
              v                                   v
+---------------------------+       +---------------------------+
|     IngestionService      |       |        QuotaService       |
| creates ingestion job     |       | check daily token quota   |
+-------------+-------------+       +-------------+-------------+
              |                                   |
              v                                   v
+---------------------------+       +---------------------------+
|          Parser           |       |      HybridRetriever      |
| PDF page / MD line / TXT  |       | tenant-local search only  |
+-------------+-------------+       +-------------+-------------+
              |                                   |
              v                                   v
+---------------------------+       +-------------+-------------+
|          Chunker          |       |                           |
| chunk_size + overlap      |       v                           v
+-------------+-------------+  +----------------+     +----------------+
              |                | Vector Search  |     |  BM25 Search   |
              v                | tenant index   |     | tenant index   |
+---------------------------+  +-------+--------+     +--------+-------+
|      HashingEmbedding     |          |                       |
| deterministic local vecs  |          +-----------+-----------+
+-------------+-------------+                      |
              |                                    v
              v                         +---------------------+
+---------------------------+           |     RRF Fusion      |
|       TenantStorage       |           | combine two rankings|
| per-tenant physical data  |           +----------+----------+
+-------------+-------------+                      |
              |                                    v
              v                         +---------------------+
+---------------------------+           |    Source Chunks    |
| data/tenants/{tenant_id}/ |           | filename/page/lines |
| files/manifest.json       |           +----------+----------+
| vector_index.json         |                      |
| keyword_index.json        |                      v
+---------------------------+           +---------------------+
                                        |   AnswerGenerator   |
                                        | DeepSeek/local demo |
                                        +----------+----------+
